...
const getOpenIdAudience = async () => {
  ...
  if (!process.env.AUTH_AUDIENCE_ID)
    throw new NotAuthorisedError('Internal configuration of authentication is not complete.');
...
export const hasAuthorisation = (token: any, targetOrganisation: string) => {
  ...
  if (organisations.includes(targetOrganisation))
    return [AuthorisationLevel.OrganisationAdmin, AuthorisationLevel.OrganisationMember];
  return [];
};
