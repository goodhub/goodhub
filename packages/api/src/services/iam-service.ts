...
if (!extensionAppId)
  throw new Error("Missing AUTH_EXTENSION_ID environment variable.");
...
export const addOrganisationToUser = async (
  ...
  const existingIndex = user.organisations.findIndex(
    (o: string) => o === organisationId
  );
  if (existingIndex === -1) user.organisations.push(organisationId);

  return await updateOrganisationForUser(
    user.id,
    customPrefix,
    user.organisations,
    token
...
export const removeOrganisationFromUser = async (
  personId: string,
  organisationId: string
) => {
  if (!personId || !organisationId)
  ...
  const organisations = user.organisations.filter(
    (o: string) => o !== organisationId
  );

  return updateOrganisationForUser(
    user.id,
    organisationsKey,
    organisations,
    token
...
const getOrganisationsForUser = async (
  ...
  const url = `https://graph.microsoft.com/v1.0/users?$filter=${encodeURIComponent(
    `${prefix}PersonId eq `
  )}%27${personId}%27&$select=id,${prefix}Organisations`;
  ...
  user.organisations = user[`${prefix}Organisations`]
    ? user[`${prefix}Organisations`].split(",")
    : [];
  return user;
};

const updateOrganisationForUser = async (
  oId: string,
  prefix: string,
