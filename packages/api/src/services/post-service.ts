...
const postToFacebook = async (post: IPost) => {
  // const { social } = await getOrganisationSocialConfiguration(
  //   post.organisationId
  // );
  ...
  //   "message",
  //   post.text.blocks.reduce((message, value) => message + "\n" + value.text, "")
  // );
  // if (post.hero?.type === "graphic") {
  //   const { id } = await uploadPhotoToFacebook(
  //     post.hero.image.original,
  //     pageToken,
  //     pageId
  //   );
  //   url.searchParams.set(
...
export const postToExternalSocial = async (
  social: ISocial,
  post: IPost
