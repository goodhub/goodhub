...
const transformImage = async (original: ProcessedImage, quality: ImageQuality, dir: WorkingFolder): Promise<[ProcessedImage, sharp.OutputInfo]> => {
...
export const uploadBlob = async (id: string, body: string | Blob | ArrayBuffer, length: number, type: string) => {
  ...
  await blockBlobClient.upload(body, length, { blobHTTPHeaders: { blobContentType: type } });
