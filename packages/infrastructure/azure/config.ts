...
import { StorageAccount, SkuName, Kind, BlobContainer, Blob, BlobServiceProperties } from '@pulumi/azure-native/storage';
...
export const setupConfig = (group: ResourceGroup, coreApi: WebApp, functionsApi: WebApp, { id, simpleId, stack }: Arguments) => {
  ...
  const backstage = all([coreApi.defaultHostName]).apply(([coreApiHostName]) => new StringAsset(JSON.stringify({
    variables: {
