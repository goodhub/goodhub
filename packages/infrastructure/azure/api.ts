...
import {
  ...
  getSharedKeysOutput,
  GetSharedKeysResult,
} from "@pulumi/azure-native/operationalinsights";
...
export const setupAPI = (
  ...
  if (!sendgridAPIKey)
    throw new Error('There is no "sendgrid-api-key" in the stack config.');
  ...
  if (!graphManagementPassword)
    throw new Error(
      'There is no "graphManagementPassword" in the stack config.'
    );

  const functionB2CPassword = config.getSecret("functionB2CPassword");
  if (!functionB2CPassword)
    throw new Error('There is no "functionB2CPassword" in the stack config.');
    ...
    appLogsConfiguration: {
      ...
      logAnalyticsConfiguration: {
        ...
        sharedKey: workspaceSharedKeys.apply(
          (r: GetSharedKeysResult) => r.primarySharedKey!
        ),
      },
    },
  });
      ...
      containers: [
        {
          ...
          env: [{ name: "TOKEN", value: token.result }],
        },
      ],
      scale: {
        maxReplicas: 1,
  ...
  const storageAccountKeys = all([storage]).apply(([s]) =>
    listStorageAccountKeysOutput({
      accountName: s.name,
      resourceGroupName: group.name,
    })
  ...
  const coreApi = new WebApp(`${id}-api-core`, {
    ...
    siteConfig: {
      ...
      appSettings: [
        ...
        {
          name: "AUTH_GRAPH_MANAGEMENT_ID",
          value: b2cConfig.graphManagementId,
        },
        {
          name: "AUTH_GRAPH_MANAGEMENT_PASSWORD",
          value: graphManagementPassword,
        },
        { name: "SENDGRID_APP_KEY", value: sendgridAPIKey },
        { name: "DB_DATABASE", value: coreDb.name },
        {
          name: "DB_USER",
          value: interpolate`${administratorLogin}@${dbServer.fullyQualifiedDomainName}`,
        },
        { name: "DB_PASSWORD", value: administratorLoginPassword },
        {
          name: "DB_HOST",
          value: dbServer.fullyQualifiedDomainName as Output<string>,
        },
