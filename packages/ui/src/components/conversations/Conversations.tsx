...
const Conversations: FC<ConversationsProps> = () => {
  ...
  return (
    <Page title={Navigation.menu.conversations} actions={[{ name: 'Create a new discussion', onClick: () => {} }]}>
      ...
      {results ? (
            ...
            {results
              ? results.map(post => (
                  <Card className="p-5 h-fit-content space-y-3 cursor-pointer">
                    <Title size="base" tight={true} weight="semibold" className="leading-5">
                      {post.title}
                    </Title>
        ...
        <>
          <span className="flex items-center mt-6 mb-2">
            ...
            <Title size="base" className="leading-4">
              Popular
            </Title>
          </span>
