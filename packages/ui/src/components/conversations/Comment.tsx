...
import { IComment, IPostIdentity } from '../../../../shared';
...
export const Comment: FC<CommentProps> = ({ postId, comment, children, level, count, onSubmit }) => {
  ...
  const [person, addPersonToCache, initiatedPersonLookup] = usePostService(state => [
    state.people[comment.postedBy],
  ...
  return (
    <div className={`w-full mt-2 ${level === 0 ? 'mb-4' : ''}`}>
      <div className="flex items-start relative">
        <div className="flex">
          <div
            className={`mt-1 w-8 h-8 z-10 border overflow-hidden border-gray-300 ${
              comment.postedIdentity !== IPostIdentity.Organisation ? 'rounded-full' : 'rounded-lg'
            } mr-3`}
          >
            {comment.postedIdentity === IPostIdentity.Organisation ? (
              <div></div>
            ) : person?.cache?.profilePicture ? (
          ...
          {isExpanded && count ? (
            <div
              ...
              className={`absolute top-4 left-4 -ml-px w-0.5 bg-gray-200`}
            ></div>
          ) : null}
        </div>
        <div className="w-full">
          ...
          <div className="flex space-x-2 text-sm text-gray-500">
            ...
            {count ? (
              <button className="flex items-center" onClick={() => setExpanded(!isExpanded)}>
...
export const CommentForm: FC<CommentFormProps> = ({ postId, replyId, placeholder, onSubmit }) => {
  ...
  return (
    <form className="flex items-start" onSubmit={handleSubmit(postComment)}>
      <div className="w-8 h-8 mb-2 mt-1 rounded-full border mr-3 overflow-hidden border-gray-300">
        {person?.profilePicture ? (
          <img src={person?.profilePicture.thumbnail} alt={person?.profilePicture.alt} />
        ) : null}
      </div>
      <TextAreaField
        rows={rows}
        validationMessage="You need to write your comment before you can post!"
        register={register}
        name="text"
        title=" "
      ...
      />
      {comment ? (
        <Button className="mb-2 mt-1 ml-3" type="submit" mode="primary">
          Submit
        </Button>
      ) : null}
    </form>
