...
const ScheduledPosts: FC<ScheduledPostsProps> = ({ organisation }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      setPosts(
        await Promise.all(
          posts.map(async (post: IPost & { snippet?: string; author?: string }) => {
  ...
  return posts && posts?.length ? (
    <div className="mb-3">
      <CreateUpdatePostWizard
        ...
        existingPost={editPostToEdit}
        modalState={editPostToEdit ? ModalState.Open : ModalState.Closed}
        onDismiss={() => setEditPost(undefined)}
      />
