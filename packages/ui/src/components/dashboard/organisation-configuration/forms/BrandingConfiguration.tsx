...
export const SketchExample: FC = () => {
  ...
  return (
    <div>
      <div
        className="border border-gray-300 hover:border-gray-400 relative h-24 rounded-lg bg-white shadow-sm px-5 py-4 cursor-pointer grid grid-cols-1 gap-4 sm:gap-2 lg:gap-4 sm:justify-between focus:outline-none"
        onClick={handleClick}
      >
...
const Theme: FC<ThemeProps> = ({ colors, selected, setValue, register }) => {
  ...
  return (
    <div
      ...
      className={`${
        selected ? 'ring-4 ring-offset-2 ring-primary-500' : ''
      } border border-gray-300 hover:border-gray-400 relative h-24 rounded-lg bg-white shadow-sm px-5 py-4 cursor-pointer grid grid-cols-4 gap-4 sm:gap-2 lg:gap-4 sm:justify-between focus:outline-none`}
    >
      {colors.map(c => (
        <div className="flex-1 h-full rounded" style={{ backgroundColor: c }}></div>
      ))}
    </div>
  );
};
...
const BrandingConfiguration: FC<BrandingConfigurationProps> = () => {
  ...
  useEffect(() => {
    const fonts = Object.values(fontPairings).flatMap(pairing => {
      const primary = `${pairing.primary.name}:${pairing.primary.weight}${
        pairing.primary.fstyle !== 'normal' ? pairing.primary.fstyle : ''
      }`;
      const secondary = `${pairing.secondary.name}:${pairing.secondary.weight}${
        pairing.secondary.fstyle !== 'normal' ? pairing.secondary.fstyle : ''
      }`;
    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        {[
          ...
          return (
            <Theme colors={colors} selected={isEqual(colors, brandColors)} register={register} setValue={setValue} />
          );
        })}
        <SketchExample />
        <SketchExample />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 ">
        ...
        {Object.values(fontPairings).map(paring => {
          return (
            <div className="overflow-hidden border border-gray-300 hover:border-gray-400 relative h-30 rounded-lg bg-white shadow-sm px-4 py-4 cursor-pointer flex flex-col gap-2 sm:gap-1 lg:gap-2 sm:justify-between focus:outline-none">
              <span
              ...
              >
                {/* Organisations name in here */ orgFontHeading}
              </span>
              <span
                className=""
                style={{
              ...
              >
                {/* Organisations description in here */ orgFontBody}
              </span>
            </div>
          );
        })}
