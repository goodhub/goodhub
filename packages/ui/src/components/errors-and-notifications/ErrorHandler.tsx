...
const ErrorHandler: FC<ErrorHandlerProps> = () => {
  const { error, eventId, clearError } = useErrorService(state => ({
    error: state.error,
  ...
  return (
    <>
      ...
      {error ? (
        <div className="fixed inset-0 w-screen h-screen flex flex-col justify-center sm:items-center bg-white z-50">
          <div className="m-8 sm:m-0 sm:-mt-8 w-full max-w-xl">
            {getIconForErrorCode(error.code)}
            <div className="mt-3 text-center sm:mt-5">
              <h3 className="text-4xl font-bold tracking-tight text-gray-900">{getMessageForErrorCode(error.code)}</h3>
              <div className="my-5">{error.code === 401 ? null : <Error error={error} eventId={eventId} />}</div>
            </div>
          </div>
