  ...
  switch (heroStyle?.position) {
    ...
    case 'trailing':
      return {
        title: 'sm:text-5xl text-4xl font-bold',
        text: [
          'h-full w-full md:w-1/2 right-0 absolute justify-center items-center md:items-end text-center md:text-right'
        ]
      };
  }
};
...
const Hero: FC<HeroProps> = ({ hero }) => {
  ...
  return (
    <div className="flex flex-col h-96 bg-white overflow-hidden relative">
      {hero.image ? (
        <BackgroundImage
          pictureStyle={mix(heroStyle.picture, heroPosition.picture)}
          backgroundStyle={mix(heroStyle.background, heroPosition.background)}
          image={hero.image}
        >
          <div className="flex m-auto max-w-6xl w-full h-full">
            <div className={`flex flex-col px-3.5 sm:px-8 lg:px-8 ${className}`} style={{ ...style }}>
              <h1 className={heroPosition.title ? heroPosition.title : 'sm:text-6xl text-5xl font-bold'}>
                {hero.title}
              </h1>
              <h2 className="sm:text-2xl text-xl">{hero.subtitle}</h2>
              <Button to="/about" mode={heroStyle.button ? heroStyle.button : 'primary'} className="mt-4">
                Learn more
              </Button>
            </div>
          </div>
