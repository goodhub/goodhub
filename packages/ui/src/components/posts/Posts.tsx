...
const Posts: FC<PostsProps> = ({ columns = 1, orgId }) => {
  const [posts, recentlyPostedPost, setPosts, clearPosts, revalidatePosts] = usePostService(state => [
    state.posts,
    state.recentlyPostedPost,
    state.setPosts,
    state.clearPosts,
    state.revalidatePosts
  ]);
