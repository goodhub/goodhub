      ...
      <div className="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
        <div className="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
          <Card className="overflow-hidden">
            {title ? (
              <div className="py-5 px-6 w-full border-b border-gray-200">
              ...
              <thead className="bg-gray-50">
                <tr>
                  {headings.map((heading, i) => (
                    <th
                      scope="col"
                      className={`px-6 pt-4 pb-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider ${
                    ...
                    >
                      {headings.map((heading, i) => (
                        ...
                        >
                          {(() => {
                            if (!row[heading.name])
                              return <Skeleton width="100%" opacity={1 - (1 / (content.length + 1)) * (r + 1)} />;
                            ...
                            if (heading.type === HeadingType.Tag)
                              return (
                                <span className="rounded-sm font-semibold px-2 py-1 bg-primary-500 text-white">
                                  {row[heading.name]}
                                </span>
                              );
                          })()}
                      ...
                      {actions.map(action => (
                        <td className="px-6 py-3 whitespace-nowrap text-right text-sm font-medium">
                          <button
                            ...
                            className="text-primary-600 hover:text-primary-900 py-1 px-2 hover:bg-gray-50 rounded"
                          >
                            {action.name}
                          </button>
                        </td>
