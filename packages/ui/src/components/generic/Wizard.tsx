...
const Step: FC<{ step: IStep }> = ({ step }) => {
  return (
    <li className="list-none" key={step.id}>
      {step.complete ? (
        <div className="group">
          <span className="flex items-start">
            ...
            <span className="ml-3 text-sm font-medium text-gray-500 group-hover:text-gray-900">{step.id}</span>
          </span>
        </div>
      ) : step.current ? (
          ...
          <div className="flex w-screen sm:w-modal h-screen sm:h-4xl sm:max-h-modal sm:max-w-7xl">
            {!hasStarted ? (
              <div className="flex flex-col overflow-y-scroll text-center mx-auto w-full max-w-3xl text-gray-700 p-8">
                ...
                {introduction}
                <Button className="w-max-content mt-6 mx-auto" mode="primary" onClick={() => setHasStarted(true)}>
                  Get started
                </Button>
              </div>
            ) : (
                ...
                <div className="bg-white border border-gray-200 shadow-sm min-w-max-content hidden sm:flex flex-col sm:p-6">
                  <div className="flex w-full">
                    <div className="flex flex-col flex-grow m-2">
                      <Title size="lg" className="sm:mb-2 sm:text-xl opacity-75">
                        {name}
                      </Title>
                      {decoration ? <div className="hidden sm:block w-56 mt-2 mb-4 p-2">{decoration?.()}</div> : null}
                      ...
                      <div className="flex flex-col flex-grow m-2">
                        ...
                        <div className="flex mt-2">
                          {steps.map((s, i) => (
                            <span
                              key={s.id}
                              className={`h-2 flex-1 mr-5 rounded-lg ${
                                i !== currentStepIndex ? 'bg-gray-100' : 'bg-primary-500'
                              }`}
                            ></span>
                          ))}
                        </div>
                  ...
                  <div className="fixed bottom-0 w-full sm:relative bg-white border-t border-gray-200 shadow-sm p-4 flex justify-between">
                    ...
                    <span>
                      {currentStepIndex !== steps.length - 1 ? (
                        <Button className="" mode="primary" onClick={next}>
                          Next
                        </Button>
                      ) : (
                        <Button mode="primary" onClick={onComplete}>
                          Finish
                        </Button>
                      )}
                    </span>
