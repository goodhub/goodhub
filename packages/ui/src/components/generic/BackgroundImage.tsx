...
const BackgroundImage: FC<BackgroundImageProps> = ({ blur = 24, image, children, backgroundStyle, pictureStyle }) => {
  ...
  return (
    <div className={`w-full h-full ${backgroundStyle?.[0]}`} style={{ ...backgroundStyle?.[1] }}>
      <div className={`w-full h-full ${pictureStyle?.[0]}`} style={{ ...pictureStyle?.[1] }}>
        ...
        <picture>
          ...
          <img
            className="absolute inset-1/2 -translate-y-1/2 -translate-x-1/2 transform animate-fadein w-full min-h-full"
            src={image.original}
            alt={image.alt}
          ></img>
        </picture>
