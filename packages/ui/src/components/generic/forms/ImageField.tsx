  ...
  return (
    <div className="mb-3 w-full flex flex-col">
      ...
      {status === Status.Idle ? (
        <div
          {...getRootProps()}
          className="mt-2 flex justify-center items-center bg-white h-36 rounded-md shadow-sm border border-gray-300 cursor-pointer overflow-hidden"
        >
          <input type="file" placeholder="" {...getInputProps()}></input>
      ...
      {status === Status.Selected ? (
        <div className="mt-2 flex flex-col justify-center items-center bg-white px-4 h-36 rounded-md shadow-sm border border-gray-300 cursor-pointer overflow-hidden">
          ...
          <div className="flex w-full">
            <div className="flex-grow relative rounded-md shadow-sm">
              <input
                ...
                value={alt}
                type="text"
                className="block w-full pr-10 flex-grow shadow-sm focus:ring-purple-500 text-sm border border-gray-300 rounded-md"
                placeholder="This is important for visually impaired users of GoodHub"
              ></input>
            </div>
            <Button className="ml-2" mode="primary" onClick={() => uploadFile()}>
              Upload
            </Button>
          </div>
        </div>
      ...
      {status === Status.Uploaded && image ? (
        <div className="mt-2 flex flex-col justify-center bg-white items-center px-4 h-36 rounded-md shadow-sm border border-gray-300 cursor-pointer overflow-hidden">
          <label className="text-sm mb-2 font-medium text-gray-700">Uploaded!</label>
          <div
            className="w-36 h-24 rounded-lg mb-4 bg-contain bg-no-repeat bg-center "
      ...
      {status === Status.Saved && image ? (
        <div
          {...getRootProps()}
          className="mt-2 flex flex-col justify-center items-center bg-white h-36 rounded-md shadow-sm border border-gray-300 cursor-pointer overflow-hidden"
        >
          <input type="file" placeholder="" {...getInputProps()}></input>
