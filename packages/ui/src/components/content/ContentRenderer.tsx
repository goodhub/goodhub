...
const applyInlineStyles = (block: any) => {
  ...
  return [
    string.slice(0, range.offset),
    '<strong>',
    string.slice(range.offset, range.length),
    '</strong>',
    string.slice(range.length)
  ].join('');
};
...
export const ContentRenderer: FC<ContentRendererProps> = ({ content }) => {
  return <div className="goodhub-content">{content.blocks.map(block => getComponentForBlock(block))}</div>;
};
