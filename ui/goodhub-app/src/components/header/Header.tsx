      ...
      <div className="flex h-16">
        <div className="flex flex-grow justify-between sm:justify-start px-2 lg:px-0">
          <Link to="/" className="flex-shrink-0 flex items-center">
            <svg className="h-8 mx-0 sm:h-8 sm:mx-1.5" viewBox="0 0 219 37">
              <defs />
              <g id="Untitled">
          ...
          <nav aria-label="Global" className="sm:ml-6 flex items-center space-x-4">
            <Menu as="div" className="relative inline-block text-left">
              <Menu.Button>
                <Button className="hidden sm:flex" onClick={() => setOpenState(!isOpen)} mode="primary">Organisations <FiChevronDown className="ml-2" /></Button>
                <Button className="flex sm:hidden" onClick={() => setOpenState(!isOpen)} mode="primary">
                ...
                >
                  {organisations ? organisations.map(o => {
                    return <Menu.Item onClick={() => {
                      setOpenState(false);
                      history.push(`/dashboard/${typeof o !== 'string' ? o.id : o}`);
                    }}>
        ...
        <div className="flex items-center md:hidden">
          <Button mode="primary">
            ...
            <FiMenu className="w-6 h-6 -m-0.5 text-white" />
          </Button>
        </div>
