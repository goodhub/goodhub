  ...
  return <Modal state={ModalState.Open} onDismiss={dismiss} padding="">
    ...
    <div className="flex flex-col overflow-y-scroll space-y-4 px-8 py-12 items-center text-gray-700 w-screen min-h-screen-safe h-screen sm:h-fit-content sm:max-h-modal sm:max-w-lg">
      ...
      <ul className="list-disc ml-8 text-sm">
        ...
        <li>You are only consenting to be contacted in relation to volunteering for this specific [organisation|project]. You won't get emails or texts about anything else.</li>
        <li>You can choose to revoke this consent at any point right here on GoodHub.</li>
      </ul>
      <button onClick={signUp} className={`flex items-center justify-center leading-6 p-4 px-8 ${state !== State.Successful ? 'bg-gray-100 hover:bg-primary-200 text-gray-700 hover:text-primary-800' : 'bg-primary-500 text-white'} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors w-full rounded-md`}>
        { (() => {
           if (state === State.Idle) return <p>Allow <span className="font-bold">{organisationName}</span> to contact you about volunteering.</p>
           if (state === State.Loading) return <Spinner size="8" />
           if (state === State.Successful) return <p className="font-bold">🎉 Success! You are signed up to volunteer at {organisationName}!</p>
        })()}
      </button>
    </div>
  </Modal>
