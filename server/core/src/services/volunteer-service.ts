...
export const createVolunteerContract = async (personId: string, candidate: Partial<IVolunteer>) => {
  ...
  try {
    const contract = await Volunteer.create({ ...candidate, id: v4(), volunteerId: personId, status: IVolunteerStatus.Open, expiry: moment().add(90, 'days').toDate() });
...
export const getVolunteersForProject = async (projectId: string) => {
  ...
  try {
    const contracts = await Volunteer.findAll({ where: { activityId: projectId } });
    return contracts.map(c => c.toJSON());
  } catch (e) {
    Sentry.captureEvent(e);
    throw new DatabaseError('Could not get these volunteers.');
...
export const getContractsForPerson = async (personId: string) => {
  ...
  try {
    const contracts = await Volunteer.findAll({ where: { volunteerId: personId } });
    return contracts.map(c => c.toJSON());
  } catch (e) {
    Sentry.captureEvent(e);
